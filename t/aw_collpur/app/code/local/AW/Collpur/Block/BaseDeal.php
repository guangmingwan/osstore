...
class AW_Collpur_Block_BaseDeal extends Mage_Core_Block_Template {
    ...
    protected function _lookUpCache($image) {
        ...
        if (file_exists($root.'/'.$cachePath)) {  
            return $cachePath;
        } else if (file_exists($root.'/'.$basePath)) {
    ...
    public function initMediaPath($image=NULL,$cacheDir=NULL,$mode=NULL,$router=NULL,$web = NULL) {
        ...
        if(!$cacheDir && !$mode && !$router) {
            $path = 'media'.'/'.AW_Collpur_Helper_Data::PREFIX_MODULE . '/' . AW_Collpur_Helper_Data::ROUTER . '/' . $image;
        }
        else if($image && $router) {            
            $path = 'media'.'/'.AW_Collpur_Helper_Data::PREFIX_MODULE . '/' . $router . '/' .$image;
        }
        else if($image) {
            $path =  'media'.'/'.AW_Collpur_Helper_Data::PREFIX_MODULE . '/' . AW_Collpur_Helper_Data::ROUTER . '/' . $cacheDir. '/' .$mode. '/'.$image;
        }
        else {
           $path =  'media'.'/'.AW_Collpur_Helper_Data::PREFIX_MODULE . '/' . AW_Collpur_Helper_Data::ROUTER . '/' . $cacheDir. '/' .$mode;
        }
     ...
     protected function _initVarienImage($basePath, $imageName, $cacheDir, $mode, $root) {
        ...
        try {
            $image = new Varien_Image($root.'/'.$basePath);
            ...
            $image->save($root.'/'.$cacheDir, $imageName);
