...
class MageWorx_CustomOptions_Model_Relation extends Mage_Core_Model_Abstract {
    ...
    public function changeHasOptions(array $groups) {
        if ($groups) {
            ...
            foreach ($groups as $groupId => $status) {
                $productIds = $this->getResource()->getProductIds($groupId);
                if ($productIds) {
                    foreach ($productIds as $productId) {
//                        if ($status == MageWorx_CustomOptions_Helper_Data::STATUS_VISIBLE) {
//                            $this->getResource()->setHasOptions($productId);
//                        } else {
//                            $this->getResource()->changeHasOptionsKey($productId);
//                        }
                        $productOptionModel->checkHasAndRequiredOptions($productId);
                    }
                }
            }
        }
