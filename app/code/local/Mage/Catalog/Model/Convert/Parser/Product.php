...
/**
    ...
    public function unparse()
    {
        ...
        // get all our custom options, so we can get them out later
        ...
        foreach ($entityIds as $i => $entityId) {
            ...
            foreach ($product->getData() as $field => $value) {
                ...
                elseif (is_array($value)) {
                	// Arrayed options, such as Tier Pricing
                	switch($field) {                		
                		case 'tier_price': 
                			$field .= 's'; // so there are no conflicts when importing
                			$lsResult = null;               			
                			if(count($value)) {
                				foreach($value as $tierprice) {
                					$lsResult .= $tierprice['price_qty'] . '=' . $tierprice['price'] . '|';
                				}
                				// remove the last pipe character
                				$lsResult = substr($lsResult,0,-1);
                				// add it in
                				$row[$field] = $lsResult;
                			}                			                			
                			break;
            ...
            /* 
             * Get custom options and put them in a format we can use
             */            
            ...
            // Create the output
            foreach($optionArray[$productSku] as $opt) {
            	// initialise variables
            	$label = '';
            	$value = '';
            	// build the label
	            $label = $opt['store_title'] 
	            	. ':'
	            	. $opt['type']
	            	. ':'
	            	. $opt['is_require']
	            ...
	            // there are no values
	            if(count($opt['values']) == 0) {
	            	// text field, no value needed
	            	$value = ':'
	            		. $opt['price_type'] 
	            		. ':' 
	            		. $opt['store_price'] 
	            ...
	            foreach($opt['values'] as $val) {
	            	if(strlen($value)) $value .= '|'; // separate values	            	
	            	$value .= $val['store_title']
	            		. ':'
	            		. $val['store_price_type']
	            		. ':'
	            		. $val['store_price']
	            		. ':'
	            		. $val['sku']
