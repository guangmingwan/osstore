 ...
 */
class MageAsia_LBTags_Block_Product_Toolbar extends Mage_Core_Block_Template
{
    /**
     * Products collection
     *
	...
	public function getPageTagUrlHtml(){
		...
		/*	if($this->idlist!=null){
			...
			$route = Mage::helper('lbtags')->getRoute();
			 ...
			 $pages = ceil( $this->getTotalNum() / (int)$this->getLimit() );
			 
			$links = "<div  class='pagerlink pages'>";
			$links = $links .  ' <span class="pagetotal" > Total:'.$this->getTotalNum().'  Pager:'.$currentPage."/".$pages.'</span><ol>';
			if ($currentPage > 1)
			{
				$links = $links . '<li><a rel="nofollow" href="' . Mage::helper("lbtags")->makeTagsViewPageUrl( $identifier_code ,(($currentPage - 1))) . '" > < <   </a> </li>';
			}else{
				$links = $links . '<li><a rel="nofollow" href="#" > < <   </a></li> ';
			}
				for($i= $this->_frameStart;$i<= $this->_frameEnd;$i++)
				{
					if($i==1){
						$followcode="";
					}else{
						$followcode='rel="nofollow"';	
					}$active="";
					if($i==$currentPage){ $active='current';}
					$links.='<li class="'.$active.'"><a '.$followcode.' class=" pagebb " href="' . Mage::helper("lbtags")->makeTagsViewPageUrl( $identifier_code ,$i) . '" > '.$i.'   </a></li>';	
				}
			
			if ($currentPage < $pages)
			{
				$links = $links .  '<li> <a class="next" rel="nofollow" href="' .  Mage::helper("lbtags")->makeTagsViewPageUrl($identifier_code , (($currentPage + 1))) . '" > > ></a></li>';
			}else{
					#$links = $links .  ' <a class="next" href="#" > > ></a></li>';
			}
			return  $links.'</ol></div>';
